...
class VirtualPostfixMachine:
    ...
    def load_module(self):
        ...
        try:
            ...
            console.print(f"ERROR: Module file '{filename}' not found.")
            exit(1)
        ...
        for line in lines:
            ...
            if current_section == "vars":
                ...
                if len(parts) != 2:
                    console.print(f"ERROR: Invalid variable declaration: {line}")
                    exit(1)
                ...
                if parts[0] == ")" or parts[0] == "(":
                    continue
                if len(parts) != 2:
                    console.print(f"ERROR: Invalid label declaration: {line}")
                    exit(1)

                label_name = parts[0].strip()
                if label_name in label_set:
                    console.print(f"ERROR: Duplicate label declaration: {line}")
                    exit(1)
                else:
                ...
                try:
                    ...
                    console.print(f"ERROR: Label value must be a non-negative integer: {line}")
                    exit(1)
                ...
                if len(parts) != 3:
                    console.print(f"ERROR: Invalid function declaration: {line}")
                    exit(1)
                function_name, function_type, n_params = parts[0], parts[1], int(parts[2])
                if function_type not in ("int", "float", "bool", "string", "void"):
                    console.print(f"ERROR: Invalid function type for {function_name}: {function_type}")
                    exit(1)
                ...
                if len(parts) != 2 or parts[1] not in supported_tokens:
                    console.print(f"ERROR: Unsupported instruction: {line}")
                    exit(1)
        ...
        while i < len(self.raw_tokens):
            ...
            if tok_type == "label" and i + 1 < len(self.raw_tokens) and self.raw_tokens[i + 1][1] == "colon":
                ...
                if self.symbolic_labels:
                    self.labels[token] = i
                self._debug_print(f"Found label: {token}. Instruction index {i}; line {i+1+self.code_start_num}; module {self.module} ")
                ...
                if tok_type == "string":
                    if token.startswith('"') and token.endswith('"'):
                        ...
                        console.print(f"\nERROR: String literal {token} must be enclosed in double quotes. Instruction index {i}; line {i+1+self.code_start_num}; module {self.module} ")
                        exit(1)
                elif tok_type == 'float':
                    try:
                        token = float(token)
                    except ValueError:
                        console.print(f"\nERROR: Invalid value for float: {token}. Instruction index {i}; line {i+1+self.code_start_num}; module {self.module}")
                        exit(1)
                elif tok_type == 'int':
                    try:
                        token = int(token)
                    except ValueError:
                        console.print(f"\nERROR: Invalid value for int: {token}. Instruction index {i}; line {i+1+self.code_start_num}; module {self.module}")
                        exit(1)
                elif tok_type == 'bool':
                    if token.upper() == 'TRUE':
                        token = True
                        ...
                        console.print(f"\nERROR: Invalid value for bool: {token}. Instruction index {i}; line {i+1+self.code_start_num}; module {self.module}")
                        exit(1)
                self.instructions.append((token, tok_type))
            i += 1
    ...
    def _get_var_type(self, var: str) -> str:
        ...
        console.print(f"\nERROR: Unknown variable: {var}; line {self.pc -1 + self.code_start_num}, module {self.module}")
        exit(1)
    ...
    def _get_value(self, var: str) -> (Any, str):
        if self.mode == 'main':
            if var in self.variable_types:
                ...
                if value is None:
                    console.print(f"\nERROR: Use of uninitialized variable: {var}; line {self.pc -1 + self.code_start_num}, module {self.module}")
                    exit(1)
                return value, type
        else:
            if var in self.variable_types:
    ...
    def _get_1_operand(self, op: str) -> (Any, str):
        try:
            ...
            console.print(f"\nERROR: Insufficient operands for operation {op}")
            exit(1)
    ...
    def run(self, debug: bool = False):
        ...
        if self.debug:
            console.print("List of instructions:")
            for i, (t, typ) in enumerate(self.instructions):
                console.print(f"  {i}: {t} {typ}")
            console.print("Label table:", self.labels)

        while self.pc < len(self.instructions):
            token, tok_type = self.instructions[self.pc]
            self._debug_print(f"\n[PC={self.pc}] Processing instruction: {token} {tok_type}")
            ...
            self._debug_print(f"  Stack after execution: {self.stack}")
            self._debug_print(f"  Variables: {self.variable_values}")
            self._debug_print(f"  Global variables: {self.globals}")
            self.pc += 1
    ...
    def _do_assign(self):
        ...
        l_lexeme, l_token = self.stack.pop()
        if l_token != "l-val":
            console.print(f"\nERROR: Cannot assign to non l-val: {l_lexeme} ({l_token})")
            exit(1)

        if r_token == "r-val":
            r_lexeme, r_token = self._get_value(r_lexeme)
            if r_token != self.variable_types[l_lexeme]:
                console.print(f"\nERROR: Type mismatch in assignment: {l_lexeme} ({self.variable_types[l_lexeme]}) and {r_lexeme} ({r_token}); line {self.pc + self.code_start_num}, module {self.module}")
                exit(1)
            ...
            try:
                ...
                if l_token != r_token:
                    console.print(f"\nERROR: Type mismatch in assignment: {l_lexeme} ({l_token}) and {r_lexeme} ({r_token}); line {self.pc + self.code_start_num}, module {self.module}")
                    exit(1)
                self._set_value(l_lexeme, r_lexeme)
            except KeyError:
                console.print(f"\nERROR: Unknown variable: {l_lexeme}; line {self.pc + self.code_start_num}, module {self.module}")
                exit(1)
    ...
    def _do_math(self, op: str):
        if op == "NEG":                         # unary minus
            ...
            if token not in ("int", "float"):
                console.print(f"\nERROR: Unary minus can only be applied to numeric types, not to {token}")
                exit(1)

            result = -lexeme
            self.stack.append((result, token))
            self._debug_print(f"  Applying unary minus: -({lexeme}) = {result}")
            ...
            if l_type != r_type:
                console.print(f"\nERROR: Type mismatch in arithmetic operation: {l_type} and {r_type}; line {self.pc + self.code_start_num}, module {self.module}")
                exit(1)
            if op == "^" and l_type != 'float' and r_type != 'float':
                console.print(f"\nERROR: Exponentiation (^) requires both operands to be float")
                exit(1)
            ...
            self._debug_print(f"  Computed expression: {l_lexeme} {op} {r_lexeme} = {result}")

    def _do_relational(self, op: str):
        l_lexeme, l_type, r_lexeme, r_type = self._get_2_operands(op)

        if l_type not in ("int", "float", "bool") or r_type not in ("int", "float", "bool"):
            console.print(f"\nERROR: Relational operator can only be applied to numbers and bool types, received types: {l_type} and {r_type}")
            exit(1)

        if l_type != r_type:
            console.print(f"\nERROR: Type mismatch in relational operation: {l_type} and {r_type}")
            exit(1)
        ...
        self._debug_print(f"  Computed relation: {l_lexeme} {op} {r_lexeme} -> {result}")

    def _convert_type(self, op: str):
        value, token = self._get_1_operand(op)
        if op == "i2f":
            if token != "int":
                console.print(f"\nERROR: i2f conversion can only be applied to int, not to {token}")
                exit(1)
            converted = float(value)
            self.stack.append((converted, "float"))
            self._debug_print(f"  Converting int to float: {value} -> {converted}")
        elif op == "f2i":
            if token != "float":
                console.print(f"\nERROR: f2i conversion can only be applied to float, not to {token}")
                exit(1)
            converted = int(value)
            self.stack.append((converted, "int"))
            self._debug_print(f"  Converting float to int: {value} -> {converted}")
        elif op == "i2s":
            if token != "int":
                console.print(f"\nERROR: i2s conversion can only be applied to int, not to {token}")
                exit(1)
            converted = str(value)
            self.stack.append((converted, "string"))
            self._debug_print(f"  Converting int to string: {value} -> {converted}")
        elif op == "s2i":
            if token != "string":
                console.print(f"\nERROR: s2i conversion can only be applied to string, not to {token}")
                exit(1)
            try:
                converted = int(value)
            except ValueError:
                ...
                exit(1)
            self.stack.append((converted, "int"))
            self._debug_print(f"  Converting string to int: {value} -> {converted}")
        elif op == "f2s":
            if token != "float":
                console.print(f"\nERROR: f2s conversion can only be applied to float, not to {token}")
                exit(1)
            converted = str(value)
            self.stack.append((converted, "string"))
            self._debug_print(f"  Converting float to string: {value} -> {converted}")
        elif op == "s2f":
            if token != "string":
                console.print(f"\nERROR: s2f conversion can only be applied to string, not to {token}")
                exit(1)
            try:
                converted = float(value)
            except ValueError:
                ...
                exit(1)
            self.stack.append((converted, "float"))
            self._debug_print(f"  Converting string to float: {value} -> {converted}")
        elif op == "i2b":
            if token != "int":
                console.print(f"\nERROR: i2b conversion can only be applied to int, not to {token}")
                exit(1)
            converted = bool(value)
            self.stack.append((converted, "bool"))
            self._debug_print(f"  Converting int to bool: {value} -> {converted}")
        elif op == "b2i":
            if token != "bool":
                console.print(f"\nERROR: b2i conversion can only be applied to bool, not to {token}")
                exit(1)
            converted = int(value)
            self.stack.append((converted, "int"))
            self._debug_print(f"  Converting bool to int: {value} -> {converted}")

    def _do_out(self):
        lexeme, token = self._get_1_operand("OUT")
        print(lexeme)
        self._debug_print(f"  Printed: {lexeme}")

    def _do_input(self):
        value = input(f": ")
        self.stack.append((value, "string"))
        self._debug_print(f"  Input: {value}")

    def _do_colon(self):
        #label, _ = self.stack.pop()
        lexeme, token = self.stack.pop()
        if token != "label":
            console.print(f"\nERROR: Expected label for 'colon' operator, found: {lexeme}")
            exit(1)
        else:
            pass
            # self.pc += 1
    ...
    def _do_jump_if_false(self):
        ...
        self._debug_print(f"  JF operator: argument 1 - label {label}, argument 2 - boolean value {condition}")
        if not condition:
            if label not in self.labels:
                console.print(f"\nERROR: Unknown label for JF jump: {label}")
                exit(1)
            self.pc = self.labels[label]
            self._debug_print(f"  Jumping to label '{label}' instruction index {self.pc}.")
        else:
            self.pc += 1  # advance to the next instruction
            self._debug_print("  No jump. Executing next instruction (no jump).")

    def _do_jump(self):
        # Unconditional jump: pop label operand and set pc.
        label, _ = self.stack.pop()
        if label not in self.labels:
            console.print(f"\nERROR: Unknown label for JMP jump: {label}")
            exit(1)
        self._debug_print(f"  JMP operator: argument 1 - label {label}")
        self.pc = self.labels[label]
        self._debug_print(f"  Unconditional jump to label '{label}' instruction index {self.pc}")

    def _do_logical(self, op: str):
        if op == "NOT":
            lexeme, token = self._get_1_operand(op)
            if token != "bool":
                console.print(f"\nERROR: NOT can only be applied to boolean types, received type: {token}")
                exit(1)
            result = not (lexeme == True)
            self.stack.append((result, "bool"))
            self._debug_print(f"  Computed logical expression: NOT {lexeme} -> {result}")
        elif op in ("AND", "OR"):
            l_lexeme, l_type, r_lexeme, r_type = self._get_2_operands(op)

            if l_type != "bool" or r_type != "bool":
                console.print(f"\nERROR: Logical operation can only be applied to boolean types, received types: {l_type} and {r_type}")
                exit(1)
            if op == "AND":
            ...
            self._debug_print(f"  Computed logical expression: {l_lexeme} {op} {r_lexeme} -> {result}")
        else:
    ...
    def _do_cat(self, op: str):
        ...
        if l_type != "string" or r_type != "string":
            console.print(f"\nERROR: Concatenation can only be applied to strings, received types: {l_type} and {r_type}; line {self.pc + self.code_start_num}, module {self.module}")
            exit(1)

        result = str(l_lexeme) + str(r_lexeme)
        self.stack.append((result, "string"))
        self._debug_print(f"  String concatenation: {l_lexeme} + {r_lexeme} -> {result}")
    ...
    def _do_stack(self, op: str):
        if op == "POP":
            if not self.stack:
                ...
                exit(1)
            lexeme, token = self.stack.pop()
            self._debug_print(f"  Popped from stack: {lexeme} ({token})")
        elif op == "DUP":
            if not self.stack:
                console.print(f"\nERROR: Cannot DUP — stack is empty")
                exit(1)
            lexeme, token = self.stack[-1]
            self.stack.append((lexeme, token))
            self._debug_print(f"  Duplicating top stack element: {lexeme} ({token})")
        elif op == "SWAP":
            if len(self.stack) < 2:
                console.print(f"\nERROR: Cannot SWAP — stack has fewer than two elements")
                exit(1)
            ...
            self._debug_print(f"  Swapping top two stack elements: {lexeme1} ({token1}) <-> {lexeme2} ({token2})")
        elif op == "NOP":
            self._debug_print("  Doing nothing (NOP)")
        ...
        if n_params > 0:
            ...
            for parameter_var, potential_value in zip(func_executor.variable_types, stack_top):
                ...
                if potential_value_type != parameter_var_type:
                    console.print(f"\nERROR: Type mismatch for function parameter {func_name}: {parameter_var} ({parameter_var_type}) and {potential_value} ({potential_value_type})")
                    exit(1)
    ...
    def _func_return(self, token: str):
        if self.parent:
            ...
            if ret_type == "void":
                self._debug_print(f"  Function {self.module.split('$', 1)[1]} returns no value (void)")
            else:
                lexeme, token = self._get_1_operand(token)
                if token != ret_type:
                    console.print(
                        f"\nERROR: Type mismatch in function return value: Expected {ret_type}, received {token}")
                    exit(1)
                self.parent.stack.append((lexeme, token))
                self._debug_print(f"  Function {self.module.split('$', 1)[1]} returns value: {lexeme} ({token})")
        else:
            console.print(f"\nERROR: Unknown function for return: {token}")
            exit(1)
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(args.path):
        console.print(f"Error during PSM initialization: Folder '{args.path}' not found.")
        exit(1)
    if not os.path.isfile(os.path.join(args.path, args.module + ".postfix")):
        console.print(f"Error during PSM initialization: File '{args.module}.postfix' not found in folder '{args.path}'.")
        exit(1)
